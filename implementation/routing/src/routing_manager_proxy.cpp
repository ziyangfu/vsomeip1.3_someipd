...
void routing_manager_proxy::init() {
    ...
    // 创建一个本地节点，并用map存起来，client id作为查找值
    // 发送节点 local_client_endpoint_impl
    sender_ = create_local(VSOMEIP_ROUTING_CLIENT);  // VSOMEIP_ROUTING_CLIENT值为0

    std::stringstream its_client;
    // 接收节点
    // 这里的client_是application_impl从配置文件中读出来的
    its_client << VSOMEIP_BASE_PATH << std::hex << client_;   // 如 /tmp/vsomeip-1443 
#ifdef WIN32
...
void routing_manager_proxy::offer_service(client_t _client, service_t _service,
        ...
        ttl_t _ttl) {
    // 如果已经连接，则直接发，如果没有连接，则先存起来
    // proxy可以是server，所以也会有offer service
    if (is_connected_) {
...
void routing_manager_proxy::on_disconnect(std::shared_ptr<endpoint> _endpoint) {
    ...
    if (!is_connected_) {
        host_->on_event(event_type_e::ET_DEREGISTERED);  // 发送给host断开连接消息
    }
}
...
void routing_manager_proxy::on_message(const byte_t *_data, length_t _size,
    ...
    if (_size > VSOMEIP_COMMAND_SIZE_POS_MAX) {  // 大于 5
        its_command = _data[VSOMEIP_COMMAND_TYPE_POS];   // data[0]
        std::memcpy(&its_client, &_data[VSOMEIP_COMMAND_CLIENT_POS],  // data[1]
                sizeof(its_client));
        std::memcpy(&its_length, &_data[VSOMEIP_COMMAND_SIZE_POS_MIN],  // data[3]
                sizeof(its_length));
        ...
        case VSOMEIP_SEND: {
            ...
            deserializer_->set_data(&_data[VSOMEIP_COMMAND_PAYLOAD_POS],  // data[7]
                    its_length);
            std::shared_ptr<message> its_message(
                    deserializer_->deserialize_message());
            if (its_message) {
                its_message->set_instance(its_instance);
                host_->on_message(its_message);  // 为什么要发给host？send是谁发给谁的？
            } else {
        ...
        case VSOMEIP_ROUTING_INFO:    // 收到了stub发来的路由广播信息
...
void routing_manager_proxy::register_application() {
    ...
    std::memcpy(&its_command[VSOMEIP_COMMAND_CLIENT_POS], &client_,  // VSOMEIP_COMMAND_CLIENT_POS == 1
            sizeof(client_));
    std::memset(&its_command[VSOMEIP_COMMAND_SIZE_POS_MIN], 0,  // 第三个位置
            sizeof(uint32_t));

    if (is_connected_) {
        (void)sender_->send(its_command, sizeof(its_command));  // 给stub发送注册消息
...
#else
            boost::asio::local::stream_protocol::endpoint(its_path.str()), // 使用UDS协议
#endif
            io_);

    local_endpoints_[_client] = its_endpoint;  // 把本地节点用map存起来
