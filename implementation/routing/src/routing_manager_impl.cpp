...
// 对于路由来说的，包括了stub的初始化以及routing_manager的初始化
void routing_manager_impl::init() {
    ...
    stub_->init();  // stub初始化，实例化一个local_server_endpoint_impl
    // 为什么要发送消息给自己，传递事件是什么意思？
    // We need to be able to send messages to ourself (for delivering events)
    // 实例化一个local_client_endpoint_impl
    (void)create_local(VSOMEIP_ROUTING_CLIENT);  // VSOMEIP_ROUTING_CLIENT = 0
    ...
    if (configuration_->is_service_discovery_enabled()) {
        ...
        std::shared_ptr<sd::runtime> *its_runtime =
                static_cast<std::shared_ptr<sd::runtime> *>(utility::load_library(
                        VSOMEIP_SD_LIBRARY,
                        VSOMEIP_SD_RUNTIME_SYMBOL_STRING));  // 加载SD库，为什么要这么做？
...
void routing_manager_impl::start() {
    ...
    if (discovery_)
        discovery_->start();
    // 回调事件注册，实际是application_impl中的on_event
...
void routing_manager_impl::offer_service(client_t _client, service_t _service,
    ...
    std::shared_ptr<serviceinfo> its_info;
    {
        ...
        local_services_[_service][_instance] = _client;  // 先存起来
...
std::shared_ptr<endpoint> routing_manager_impl::create_server_endpoint(
    ...
    try {
        boost::asio::ip::address its_unicast = configuration_->get_unicast();  // 拿到单播地址
        if (_reliable) {
            its_endpoint = std::make_shared<tcp_server_endpoint_impl>(   // 实例化一个TCP server endpoint
                    shared_from_this(),
...
std::shared_ptr<endpoint> routing_manager_impl::find_local(client_t _client) {
    std::lock_guard<std::recursive_mutex> its_lock(endpoint_mutex_); // 这里是可重入锁
    std::shared_ptr<endpoint> its_endpoint;
...
void routing_manager_impl::add_routing_info(service_t _service,
    ...
    if (specific_endpoint_clients.find(get_client()) == specific_endpoint_clients.end()) {
        ...
        if (its_endpoint->is_connected()) {
            host_->on_availability(_service, _instance, true);  // 回调应用程序server端 on_availability函数
        }
    }
    ...
    for (client_t client : specific_endpoint_clients) {
        ...
        if (its_endpoint->is_connected()) {
            host_->on_availability(_service, _instance, true); // // 回调应用程序client端 on_availability函数
        }
    }
    ...
    for (auto i : its_events) {
        for (auto j : i.second) {
            for (auto k : j.second) {
                ...
                events_[i.first][j.first][k] = its_event;  // 把事件记录下来
            }
        }
    }
                ...
                for (auto l : k.second) {
                    ...
                    if (its_event) {
                        eventgroups_[i.first][j.first][k.first]->add_event(
                                its_event);  // 把事件添加到事件组
                        its_event->add_eventgroup(k.first);  // 这个事件属于哪个事件组
                    }
                }
            }
        }
