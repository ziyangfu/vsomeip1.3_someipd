...
bool application_impl::init() {
    ...
    // Application name 如果命令行中有名字，则使用命令行中的名字，并根据名字来确定是客户端还是服务端
    if (name_ == "") {
    ...
    if (configuration_) {
        client_ = configuration_->get_id(name_);  // clientId是从配置文件中读取的

        // Routing
        // 若选中本应用为路由，则实例化管理器，若不是，则实例化路由代理
        if (name_ == configuration_->get_routing_host()) {
            routing_ = std::make_shared<routing_manager_impl>(this);   // 路由
        } else {
            routing_ = std::make_shared<routing_manager_proxy>(this);  // 路由代理
        }
    ...
    if (is_initialized_) {
        ...
        signals_.async_wait(its_signal_handler);  // 调用boost asio接口
    }
...
void application_impl::stop() {
    ...
    for (auto &t : dispatchers_)  // 停掉所有线程
        t.join();
...
void application_impl::register_availability_handler(service_t _service,
    ...
    availability_[_service][_instance] = _handler;  // 把这个handle记录下来
}
