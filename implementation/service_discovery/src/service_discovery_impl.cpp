...
void service_discovery_impl::init() {
    ...
    default_ = std::make_shared < service_discovery_fsm
            > ("default", shared_from_this());   // 实例化一个service_discovery_fsm

    std::shared_ptr < configuration > its_configuration =
            host_->get_configuration();
    if (its_configuration) {
        unicast_ = its_configuration->get_unicast();  // IP地址
        ...
        for (auto its_group : its_servicegroups) {
            if (its_group != "default"
                    && its_configuration->is_local_servicegroup(its_group)) {  // 本地事件组
...
void service_discovery_impl::subscribe(service_t _service, instance_t _instance,
            ...
            if (found_eventgroup != found_instance->second.end()) {
                ...
                if (found_client != found_eventgroup->second.end()) {
                    if (found_client->second->get_major() == _major) {
                        found_client->second->set_ttl(_ttl);  // 找到并将client的设置ttl
                    } else {
                        VSOMEIP_ERROR
    ...
    subscribed_[_service][_instance][_eventgroup][_client] = its_subscription;  // 描述一个订阅
...
void service_discovery_impl::insert_option(
    ...
    if (unicast_ == _address) {
        if (unicast_.is_v4()) {
            ...
            if (its_option) {
                ...
                its_option->set_udp(!_is_reliable);  // 设置了udp
    ...
    // 多播
        ...
        if (_address.is_v4()) {
            ...
            if (its_option) {
                its_option->set_address(its_address);  // 没有设置 udp
                its_option->set_port(_port);
                _entry->assign_option(its_option, 1);
            }
        } 
...
// 插入 find service entry， 这仅仅是针对 client
    ...
    for (auto its_service : _requests) {
        for (auto its_instance : its_service.second) {
            ...
            if (its_entry) {
                ...
                its_entry->set_major_version(its_request->get_major());   // 这几个参数都保存在request类中
...
// 插入 offer service entry， 这仅仅是针对 server
...
void service_discovery_impl::insert_subscription_ack(
    ...
    boost::asio::ip::address its_address;
    uint16_t its_port;
    if (_info->get_multicast(its_address, its_port)) {  // its_address是一个多播地址
...
void service_discovery_impl::process_serviceentry(
    ...
    for (auto i : { 1, 2 }) {
        for (auto its_index : _entry->get_options(i)) {
            ...
            std::shared_ptr < option_impl > its_option = _options[its_index];// 通过index拿到option
...
void service_discovery_impl::handle_service_availability(service_t _service,
    ...
    if (0 < _ttl) {
        // 添加到路由信息中，会回调应用程序 on_availability
...
void service_discovery_impl::handle_eventgroup_subscription(service_t _service,
    ...
    if (its_message) {
        ...
        if (!its_info || _major > its_info->get_major()
            ...
            its_info = std::make_shared < eventgroupinfo > (_major, 0);  // TTL设置为0，即回复nack
            is_nack = true;
        } else {
